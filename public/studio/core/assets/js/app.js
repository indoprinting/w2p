			...
			templates : function(res) {
				...
				if (res.items && res.items.length > 0) {
					res.items.map(function(item) {
						...
						html += '<li style="background-image: url(\''+item.screenshot+'\'); background-size: contain; background-repeat: no-repeat;" \
								data-ops=\'[{\
									"type": "template", \
									"name": "'+item.name+'", \
									"id": "'+item.id+'", \
				...
				if (res.items && res.items.length > 0) {
					...
					res.items.map(function(item) {
						...
						html += '<li style="background-image: url(\''+item.thumbnail_url+
							'\'); background-size: contain; background-repeat: no-repeat;" data-ops="'+oid+'" class="lumise-clipart">'+
							'<i data-tag="'+item.id+'">'+
