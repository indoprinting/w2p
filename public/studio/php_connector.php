...
class lumise_connector {
    ...
    public function __construct() {
        ...
        $this->config = array(
            ...
            "upload_path" => '/www/wwwroot/idp/w2p/w2p/public/studio/data/',
            "upload_url" => 'https://indoprinting.co.id/studio/data/',
    ...
    public function save_order(){
        ...
        // Get design id to items row
        $data_designs = mysqli_query($w2p, "SELECT id FROM nsm_order_products ORDER BY id desc LIMIT 1");
        $item_design = mysqli_fetch_array($data_designs);

        // Generate Invoice Number
        $data_invoice = mysqli_query($w2p, "SELECT id_order, no_inv FROM idp_orders ORDER BY id_order desc LIMIT 1");
        $row_query = mysqli_fetch_array($data_invoice);
        $get_invoice = $row_query['no_inv'];
        $last_invoice = basename($get_invoice);
        $no_invoice = $last_invoice;
        while($no_invoice<=$last_invoice)
        {
            $no_invoice++;
        }
        $invoice = @date ("Y/m");
        $url = 'https://indoprinting.co.id/orders-api';
        $cust_phone = $_POST['phone'];
        $data_designs = array(
            'no_inv'          => "INV-$invoice/$no_invoice",
            'cust_id'         => 0,
            'cust_name'       => $_POST['first_name'] . ' ' . $_POST['last_name'],
            'address'         => $_POST['address'] . ' ' . $_POST['city'] . ' ' . $_POST['zip'],
            'total'           => $cart_data['total'],
            'items'           => $item_design["id"],
            'pickup'          => $_POST['pickup'],
            'cust_phone'      => $_POST['phone'],
            'cust_email'      => $_POST['email'],
            'pickup_method'   => $_POST['pickup_method'],
            'payment_method'  => $_POST['payment'],
            'payment_status'  => "Waiting Transfer",
            'url_track_order' => "https://indoprinting.co.id/trackorder?inv=INV-$invoice/$no_invoice&phone=$cust_phone&submit=1",
        ...
        $options = array(
            'http' => array(
                ...
                'method'  => 'POST',
                'content' => http_build_query($data_designs),
            ),
        );
...
//            CURLOPT_URL => 'https://printerp.indoprinting.co.id/api/v1/sales?invoice=INV-"'.$invoice.'"/3600',
...
//        echo $response;
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://printerp.indoprinting.co.id/api/v1/sales',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            ...
            CURLOPT_POSTFIELDS =>'{
                ...
                "discount": 0,
                "use_transfer": 1,
                "warehouse": "'.$_POST['pickup'].'",
                "items": [
                    {
                        "code": "'.$_POST['code_bahan'].'",
                        "quantity": "'.$_POST['qty_produk'].'",
                        "width": "'.$_POST['lebar_bahan'].'",
                        "length": "'.$_POST['panjang_bahan'].'",
                        "note": "'.$_POST['catatan'].'"
                    },
                    {
                        "code": "JSED20",
    ...
    public function api_erp(){
        $url="https://printerp.indoprinting.co.id/api/v1/";
        $data=array(
            "name"     => "Sadid Test",
            "phone"    => "085692650482",
            "email"    => "idp.devops2@gmail.co.id",
            "company"  => "",
        );
        $options = array(
            "http"=> array(
                ...
                "header"=>array(
                    ...
                    "Authorization: Bearer beWHVaFrOH2c7WuHKF3jfValCfFseMjbKaVcHQdS2FNc8B_LNfpcMbM0TCi6VRxh"
                ),
    ...
    public function update() {
        ...
        if (!empty($connector_content)) {
            ...
            $connector_content = str_replace(array(
                '/www/wwwroot/idp/w2p/w2p/public/studio/data/',
                'https://indoprinting.co.id/studio/data/',
                'localhost',
                'idp_w2p',
                'Dur14n100$',
                'idp_w2p',
